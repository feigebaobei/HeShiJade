...
|-|-|-|
|分支|f_comp||
|为循环组件添加组件时未显示出来|done||
|改变循环组件的任一子组件大小时更新所有子组件。改变不能拖到组件大小了，不此功能了。|done||
|loop组件增加了itemHeight 更新文档|done||
|检查渲染侧layout是否正确运行|done||
|写一个低代码的示例|done||
|渲染完成后   =》   渲染视图后|done||
|丰富组件|||
||inputNumber||
...
|行为面板的icon放在内部|待定||
|要上生产的内容|||
|fixed: layout组件的left/right区的组件在选中是高度不应该为0|||
|fixed: 解决无法选中组件的问题。原因是在slots内的第一个组件被设置为页面的根组件。|||
|fixed: 当进入一个无页面的应用后，创建页面后不应该无法选中。原因是无页面的应用的页面树中无法增加页面节点。|||
|fixed: 当新增加的页面且不添加组件时，再进入此应用的搭建页面后应该存在此页面。|||
|feat: 官网说明循环组件中的组件的事件的回调方法的参数（所有组件都有变化）|||
|feat: behavior面板的“渲染完成后”文本改变为“渲染视图后”|||
|feat: 改变table组件的子元素时同列应该更新|||
|feat: 增加循环组件（loop）|||
|feat: loop组件的props面板的“项目本来的大小”文本改变为“项目本来的宽度”|||
|feat: loop组件的props面板增加“项目本来的高度”|||
|chore:整理出接口文档|||
||||
